...
export class ArticleService {
	...
	async findOne(slug: string, userId: number): Promise<ArticleRO> {
		...
		(article as any).isFavourite = !!(await (await this.userRepository.findOneBy({ id: userId })).favorites).find(a => a.id === article.id);
	...
	async deleteComment(slug: string, id: number): Promise<ArticleRO> {
		...
		return { article: await this.articleRepository.save(article) };
	}
